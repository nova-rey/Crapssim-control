...
def make_leaderboard(rows: List[Dict[str, Any]], metric: str, top_k: int = 10) -> List[Dict[str, Any]]:
    """
...
def make_comparisons(rows: List[Dict[str, Any]], metric: str) -> Dict[str, Any]:
    ...
    return {"metric": metric, "top_run": top.get("run_id"), "comparisons": comps, "correlations": _correlations(rows)}
