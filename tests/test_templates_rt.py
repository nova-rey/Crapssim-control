...
def test_render_and_diff_simple_mode_nonbubble():
    ...
    template = {
        "pass": "units*2",                 # 10
        "place": {"6": "units*2", "8": "units*2", "5": "units"},  # 6/8=10->legal 6/6? actually 10 -> 6*?  -> 12 after legalizer; 5->5
        "field": "units"
    }
...
def test_odds_when_point_on():
    ...
    template = {
        "pass": 10,
        "odds": {"pass": "units*3"}  # 15 raw, cap = 30, so 15 is fine
    }
...
def test_diff_idempotence_and_ordering():
    ...
    assert any(a for a in actions if a["action"] == "set" and a["bet_type"] == "pass_line" and a["amount"] == 10)
