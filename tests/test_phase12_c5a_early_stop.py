...
def test_unactionable_due_to_table_min(monkeypatch):
    ...
    monkeypatch.setattr(
        a,
        "snapshot_state",
        lambda: {"bankroll": 4.0, "point_on": False, "roll_in_hand": 1, "active_bets_sum": 0},
    )
    out = a.step_roll((1, 1))
    assert out.get("status") == "terminated"
...
def test_policy_caps_make_unactionable(monkeypatch):
    ...
    monkeypatch.setattr(
        a, "snapshot_state", lambda: {"bankroll": 50.0, "point_on": True, "active_bets_sum": 0}
    )
    out = a.step_roll((2, 3))
    assert out.get("status") == "terminated"
    assert out.get("reason") == "unactionable_bankroll"
...
def test_flags_disable_early_stop(monkeypatch):
    ...
    calls = {"rolled": 0}
    a.transport.step = lambda dice=None, seed=None: calls.__setitem__(
        "rolled", calls["rolled"] + 1
    ) or {"rolled": True}
