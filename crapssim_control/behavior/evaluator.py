...
class BehaviorEngine:
    def __init__(self, rules: List[RuleDef], verbs: Optional[VerbRegistry]=None, once_per_window: bool=True, verbose: bool=False):
        self.rules = rules
    ...
    def _decrement_scope(self, scope: str) -> None:
        for st in self._cooldowns.values():
            if scope == "roll": st.rolls = max(0, st.rolls-1)
            elif scope == "hand": st.hands = max(0, st.hands-1)
            elif scope == "point_cycle": st.point_cycles = max(0, st.point_cycles-1)

    def on_scope_advance(self, scope: str) -> None:
        self._decrement_scope(scope)

    def evaluate_window(self, window: str, snap: Dict[str,Any], journal: DecisionsJournal) -> Optional[Dict[str,Any]]:
        ...
        for r in self.rules:
            ...
            if r.cooldown:
                if (r.cooldown.get("rolls",0) and st.rolls>0) or (r.cooldown.get("hands",0) and st.hands>0) or (r.cooldown.get("point_cycles",0) and st.point_cycles>0):
                    if self.verbose:
                        journal.write(DecisionAttempt(snap.get("roll_index",0), window, r.id, "dsl", r.when, False, r.then, getattr(r,"args",{}), False, False, "COOLDOWN"))
                    continue
            # guards
            ok = True
            ...
            if not ok:
                if self.verbose:
                    journal.write(DecisionAttempt(snap.get("roll_index",0), window, r.id, "dsl", r.when, False, r.then, getattr(r,"args",{}), False, False, "GUARD_FALSE"))
                continue
            # when
            try:
                cond = _eval_bool(r.when, snap)
            except Exception:
                journal.write(DecisionAttempt(snap.get("roll_index",0), window, r.id, "dsl", r.when, False, r.then, getattr(r,"args",{}), False, False, "WHEN_EVAL_ERROR"))
                continue
            if not cond:
                if self.verbose:
                    journal.write(DecisionAttempt(snap.get("roll_index",0), window, r.id, "dsl", r.when, False, r.then, getattr(r,"args",{}), False, False, None))
                continue

            # build intent via verb registry
            intent = self.verbs.apply(r.then, getattr(r,"args",{}))
            # legality is enforced upstream; we only propose
            attempt = DecisionAttempt(snap.get("roll_index",0), window, r.id, "dsl", r.when, True, r.then, getattr(r,"args",{}), True, True, None)
