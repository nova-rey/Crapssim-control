...
def main():
    ...
    ap.add_argument(
        "--metric",
        default="ROI",
        help="Leaderboard metric (e.g., ROI, bankroll_final, hands, rolls)",
    )
    ap.add_argument("--top", type=int, default=10, help="Leaderboard size")
    ap.add_argument(
        "--compare", action="store_true", help="Write comparisons.json with deltas and correlations"
    )
    args = ap.parse_args()
    ...
    out = aggregate(
        out_dir=out_dir,
        leaderboard_metric=args.metric,
        top_k=args.top,
        write_comparisons=args.compare,
    )
